...
def load_transcriptions(trans_dir: str, commonvoice_tsv: str = None, commonvoice_clips_dir: str = None) -> dict:
    transcriptions = {}
...
def create_spleeter_csv(
    ...
    for i, stem_file in enumerate(tqdm(stem_files, desc=f"Processing {subset_dir} stems")):
        ...
        try:
            ...
            noise_path = os.path.join(subset_dir, f"{stem_name}_noise.wav")
...
def main() -> None:
    ...
    speech_dir = config['commonvoice_clips_dir']
    ...
    logging.info(f"Total stem files found: {len(stem_files)}")
